...
void read_adress(short *a, short *b, short *c, short *d) {		//citirea IP-urilor, a mastii si a adreselor de retea in formatul corespunzator

	scanf("%hd.%hd.%hd.%hd", a, b, c, d);
};
void read_network(short *a, short *b, short *c, short *d, short *e){		//citirea celor N adrese de retea 

	scanf("%hd.%hd.%hd.%hd/%hd", a, b, c, d, e);
}
struct adress task1(short MSK_2) {
	struct adress result; // in variabila 'result' se va stoca maska 2 in format zecimal
	unsigned short byte_1 = 0, byte_2 = 0 , byte_3 = 0, byte_4 = 0;  // declararea celor 4 bytes continuti de masca 2
	unsigned short aux = 1, aux2 = 0; // aux  si aux2 sunt utilizate pentru operatiile pe biti
	...
	if(completed == 1) { // cazul in care primul byte este completat numai cu biti de 1, in zecimal reprezentand numarul 255
		...
		for (short i = 0; i < rest; ++i) { 		//byte-ul este completat de la pozitia 7 spre pozitia 0 cu biti de 1, atatia cati indica variabila rest
			aux2 = 0;
			aux2 = aux2 | aux;
			aux2 = aux2 << (7 - i);
			byte_1 = byte_1 | aux2;
...
void print_task1(struct adress mask) { 		//afisarea mastii construite
	printf("-1 %hd.%hd.%hd.%hd\n", mask.a, mask.b, mask.c, mask.d);
}
short turnOctal(unsigned short number) {		//functie ce returneaza valoare in octal a unui numar dat ca parametru
	short octalNumber = 0, i = 1;
    while(number != 0) {
...
short turnBinar(unsigned short number) {		//functie ce returneaza valoarea in binar a unui numar dat ca parametru
	short binarNumber = 0, i = 1;
	while(number != 0) {
...
void task2(struct adress MSK_1) {
	printf("-2 %hd.%hd.%hd.%hd ", turnOctal(MSK_1.a), turnOctal(MSK_1.b), turnOctal(MSK_1.c), turnOctal(MSK_1.d)); //afisarea mastii 1, MSK_1, in baza 8
...
bool task6_checker(unsigned short number) { //verifica corectitudinea unui numar, pentru conditiile impuse de masca(lipseste alternanta 0/1)
	unsigned short usableBit, usableBits[8], movableBit = 1, compare = 0;
	short i = 0;
...
unsigned short task7_corrector(unsigned short number) {
	unsigned short result = 0, usableBit, usableBits[8], movableBit = 1;
	short i = 0;
	movableBit = movableBit << 7;
	while(usableBit != 0){
		usableBit = number & movableBit;
...
void task7(struct adress mask,  bool checker) {
	unsigned short corected;
	if(checker == 1) {
		printf("-7 %hd.%hd.%hd.%hd\n", mask.a, mask.b, mask.c, mask.d); //instructiune ce afiseaza MSK_1, daca aceasta este corecta, fapt verificat la task6
	} else {		/*daca un byte component al mastii prezshorta alternanta 0/1, sau nu este 255 sau 0,
...
void task9(struct adress IP, struct network networks[], short count) {
	short validNetworks[count], j = 0;
	for (short i = 0; i < count; i++) {
		...
		struct adress my_ip; //stocarea shortr-o structura doar a celor N-adrese IP, fara masca corespunzatoare
	...
	printf("-9 ");
	for (short k = 0; k < j; k++) {
		printf("%hd ", validNetworks[k]); /*afisarea pozitiilor adreselor IP din sirul de N adrese ce au adresa de retea egala 
										  cu cea dshortre IP_2 si masca fiecarei adrese*/
	}	
}
...
short main() {
	...
	bool task6_checker;
	short number_lines;
	scanf("%hd", &number_lines); //citirea numarului de seturi de date
	for (short i = 0; i < number_lines; ++i) {  /* in instructiunea for, se va citi, pe rand, fiecare set de date*/
